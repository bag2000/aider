...
├── init.conf.example    # Пример конфигурации
├── init.conf            # Общие настройки (создаётся пользователем)
...
**Перед первым запуском скопируйте `init.conf.example` в `init.conf` и заполните его своими данными.**

### 1. Подготовка конфигурации

В проекте присутствует пример конфигурационного файла `init.conf.example`. Скопируйте его в `init.conf` и отредактируйте согласно вашим параметрам:

```bash
cp init.conf.example init.conf
```

Затем отредактируйте файл `init.conf`:
...
Если файла `init.conf.example` нет, вы можете создать `init.conf` вручную с приведённым выше содержимым.
